...
if (!empty($product_id_direct)) {
    ...
    if ($seller_row = mysqli_fetch_assoc($seller_q)) {
        ...
        AND tbl_product_master.is_draft = 0
          AND tbl_product_master.user_id = '$seller_user_id'
          AND tbl_product_master.product_status NOT IN ('Completed', 'Pickup Scheduled', 'Third-Party Transaction','Withdraw')
...
AND tbl_product_master.is_draft = 0";
 ...
 if ($session_user_type == "Seller") {
    ...
    $query .= " AND tbl_product_master.product_status NOT IN ('Completed', 'Pickup Scheduled', 'Third-Party Transaction','Withdraw') ";
...
<span class="text-secondary">
    <?= 
        (float)$rw['quantity_kg'] > 0
            ? htmlspecialchars($rw['quantity_kg']) . ' (kg)'
