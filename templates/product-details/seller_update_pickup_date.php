...
if ($login == "No") {
    ...
    if ($execute == 1) {
// ✅ Fetch existing deal_status_history for this view_id (not by buyer_id + product_id)
...
// Fetch negotiation amount
$dataget = mysqli_query($con, "SELECT negotiation_amount FROM tbl_user_product_view WHERE view_id='$view_id'");
$data = mysqli_fetch_row($dataget);
$negotiation_parts = explode(',', $data[0]);
$negotiation_amount = end($negotiation_parts);

// ✅ Update the same view with both status and full history
mysqli_query($con, "
...
");
        ...
        $rejected_views_res = mysqli_query($con, "
    SELECT view_id, deal_status_history 
    FROM tbl_user_product_view 
    WHERE view_id <> '$view_id' 
      AND product_id = '$product_id' 
      AND seller_id = '$session_user_code'
");

while ($row = mysqli_fetch_assoc($rejected_views_res)) {
    $rejected_view_id = $row['view_id'];
    $history = [];

    // Decode existing JSON history
    if (!empty($row['deal_status_history'])) {
        $history = json_decode($row['deal_status_history'], true);
        if (!is_array($history)) {
            $history = [];
        }
    }

    // Append 'Offer Rejected'
    $history[] = [
        'status' => 'Offer Rejected',
        'time' => $timestamp
    ];

    $updated_history_json = mysqli_real_escape_string($con, json_encode($history));

    // Update deal_status_history
    mysqli_query($con, "
        UPDATE tbl_user_product_view SET 
            deal_status_history = '$updated_history_json'
        WHERE view_id = '$rejected_view_id'
    ");
}
...
// Fetch current history only ONCE
    $history_res = mysqli_query($con, "SELECT product_status_history FROM tbl_product_master WHERE product_id =     '$product_id'");
    $history_row = mysqli_fetch_assoc($history_res);
...
// Update product table
        ...
        $noti_details ="The pickup for ". $product_name . " has been scheduled by seller on " . $pickup_date;
