$(document).ready(function () {
    ...
    $('#continueStep1').click(function (e) {
        ...
        if (selectedCategoryId) {
            ...
            $.ajax({
                url: "templates/seller-post/step1_save_category.php",
                type: "POST",
    ...
    $('#continueStep2').click(function (e) {
        ...
        if (isValid) {
            ...
            $.ajax({
                url: "templates/seller-post/step2_save_item_details.php",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
                ...
                success: function (response) {
                    if (response.status === "DraftUpdated") {
                        // toastr.success("Step 2 data saved successfully!", "Success");
                        showStep(3);
                        getAddress();
                    } else {
     ...
     function autoSaveSingleField(fieldName, value) {
            ...
            $.ajax({
                url: "templates/seller-post/part_darft.php",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
    ...
    const formData = new FormData();
    formData.append("post_id", draftPostId); // globally available
...
$('#quantityKg').on('keyup change', function () {
    ...
    //     if (!selectedAddressId) {
    //         toastr.warning("Please select an address before saving.", "Validation Warning");
    //         return;
    //     }
    
    //     $.ajax({
    //         url: "templates/seller-post/step3_save_address.php",
    //         type: "POST",
    //         data: {
    //             post_id: draftPostId,
    ...
    //         dataType: 'json',
    //         success: function (response) {
    //             if (response.status === "PostSaved") {
    //                 toastr.success("Your post has been successfully saved!", "Success");
    
    //                 setTimeout(function () {
    //                     window.location.href = baseUrl + '/product_list';
    //                 }, 2000);
    //             } else {
    //                 toastr.error(response.status_text || "Error saving the address.", "Error");
    //             }
    //         },
    //         error: function () {
    //             toastr.error("Something went wrong while saving the address.", "Error");
    //         }
    //     });
    // });
    ...
    $.ajax({
        ...
        data: {
            post_id: draftPostId,
            address_id: selectedAddressId,
            baseUrl: baseUrl // ✅ Make sure this is defined globally
        },
        dataType: 'json',
        success: function (response) {
            if (response.status === "PostSaved") {
