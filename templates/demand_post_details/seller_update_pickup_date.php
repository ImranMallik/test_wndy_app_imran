...
if ($login == "No") {
    ...
    if ($execute == 1) {
        ...
        $dataget = mysqli_query($con, "select negotiation_amount from tbl_demand_post_item_transactions where view_id='$view_id' ");
        $data = mysqli_fetch_row($dataget);
        $negotiation_amount = $data[0];

        // UPDATE in tbl_demand_post_item_transactions
        ...
        // UPDATE in tbl_demand_post_item_transactions all request will be closed 
        mysqli_query($con, "UPDATE tbl_demand_post_item_transactions SET 
        ...
        $dataget = mysqli_query($con, "select buyer_id, assigned_collecter, seller_id from tbl_demand_post_item_transactions where view_id='" . $view_id . "' ");
        $data = mysqli_fetch_row($dataget);
