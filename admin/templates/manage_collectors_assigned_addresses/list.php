...
if ($view_permission == "Yes") {
	...
	// $under_buyer_id = mysqli_real_escape_string($con, $_POST['under_buyer_id']);
	...
	if ($searchValue != '') {
		...
		tbl_user_master.update_timestamp like '%" . $searchValue . "%' ) ";
	}
	...
	$query = "SELECT 
				...
				tbl_user_master.update_timestamp
			...
			WHERE tbl_user_master.user_type = 'Collector' AND 1";

	// collector filter
	if ($user_id != "") {
		$query .= " AND tbl_user_master.user_id = '" . $user_id . "'";
	}
	...
	while ($row = mysqli_fetch_assoc($empRecords)) {
		...
		$data[] = array(
			...
			"buyer_name" => $row['buyer_name'] . ' [' . $row['buyer_ph_num'] . ']',
