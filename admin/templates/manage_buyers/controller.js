...
function clear_input() {
  ...
  _("#aadhar_num").value = "";
  _("#gst_num").value = "";
  _("#authorization_certificate_num").value = "";
  _("#buyer_type").value = "";
  _("#active").value = "Yes";
...
function chngMode(mode) {
  if (mode == "Update") {
    _(".entry_modal_title").innerHTML = "Update Buyer Details :";
    _(".save_btn").innerHTML = "Update Data";
  } else if (mode == "Insert") {
    _(".entry_modal_title").innerHTML = "Enter Buyer Details :";
...
function show_list() {
  if (view_per == "Yes") {
    $("#data_table").DataTable({
      ...
      serverMethod: "post",
      ajax: {
        url: "templates/manage_buyers/list.php",
        data: function (d) {
      ...
      columns: [
        ...
        { data: "pan_num" },
        { data: "aadhar_num" },
        { data: "authorization_certificate_num" },
        { data: "gst_num" },
        { data: "buyer_type" },
        ...
        { data: "working_days" },
        { data: "working_method" },
        { data: "livelihood_source" },
        { data: "deal_customer" },
        { data: "ac_price" },
        { data: "washing_machine_price" },
        { data: "fridge_price" },
        { data: "tv_price" },
        { data: "microwave_price" },
        { data: "laptop_price" },
        { data: "geyser_price" },
        { data: "paper_price" },
        { data: "iron_price" },
...
function save_details() {
  ...
  if (!_("#name").checkValidity()) {
    toastr["warning"](
      "Buyer Name : " + _("#name").validationMessage,
      "WARNING"
    );
  ...
  if (!_("#email_id").checkValidity()) {
    toastr["warning"](
      "Email id : " + _("#email_id").validationMessage,
      "WARNING"
    );
  ...
  if (!_("#aadhar_num").checkValidity()) {
    toastr["warning"](
      "Aadhar Number : " + _("#aadhar_num").validationMessage,
      "WARNING"
    );
  ...
  if (!_("#authorization_certificate_num").checkValidity()) {
    toastr["warning"](
      "Authorization Certificate Number : " +
        _("#authorization_certificate_num").validationMessage,
      "WARNING"
    );
    showInputAlert(
      "authorization_certificate_num",
      "warning",
      _("#authorization_certificate_num").validationMessage
    );
  ...
  if (!_("#buyer_type").checkValidity()) {
    toastr["warning"](
      "Buyer Type : " + _("#buyer_type").validationMessage,
      "WARNING"
    );
    showInputAlert("buyer_type", "warning", _("#buyer_type").validationMessage);
    _("#buyer_type").focus();
    save_no = 0;
    return false;
  }
  ...
  if (save_no == 1 && insert_per == "Yes") {
    ...
    save_data.open("POST", "templates/manage_buyers/save_details.php", true);
    save_data.send(data);
  }
}
...
function update_data(user_id) {
  if (edit_per == "Yes") {
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == 4 && xhr.status == 200) {
        ...
        if (response["status"] == "error") {
          ...
          _("#aadhar_num").value = response["aadhar_num"];
          _("#gst_num").value = response["gst_num"];
          _("#authorization_certificate_num").value =
            response["authorization_certificate_num"];
          _("#buyer_type").value = response["buyer_type"];
    ...
    xhr.open("POST", "templates/manage_buyers/update_data_input.php", true);
    xhr.send(data);
  } else {
    toastr["error"](
...
function delete_data(user_id) {
  if (del_per == "Yes") {
    ...
    xhr.open("POST", "templates/manage_buyers/delete_data.php", true);
    xhr.send(data);
  } else {
    toastr["error"](
...
function view_addresses(user_id) {
  if (view_per === "Yes") {
    ...
    xhr.open("POST", "templates/manage_buyers/fetch_buyer_addresses.php", true);
    xhr.send(data);
  } else {
