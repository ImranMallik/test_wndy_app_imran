...
if ($view_permission == "Yes") {
	...
	if ($searchValue != '') {
		...
		tbl_user_master.aadhar_num like'%" . $searchValue . "%' or
		tbl_user_master.gst_num like'%" . $searchValue . "%' or
		tbl_user_master.user_type like'%" . $searchValue . "%' or
		tbl_user_master.active like'%" . $searchValue . "%' or
		tbl_user_master.entry_timestamp like'%" . $searchValue . "%' or
		tbl_user_master.authorization_certificate_num like'%" . $searchValue . "%' or
		tbl_user_master.buyer_type like'%" . $searchValue . "%' or
		...
		tbl_user_master.waste_cate like'%" . $searchValue . "%' or
		tbl_user_master.working_days like'%" . $searchValue . "%' or
		tbl_user_master.working_method like'%" . $searchValue . "%' or
		tbl_user_master.livelihood_source like'%" . $searchValue . "%' or
		tbl_user_master.deal_customer like'%" . $searchValue . "%' or
		tbl_user_master.ac_price like'%" . $searchValue . "%' or
		tbl_user_master.washing_machine_price like'%" . $searchValue . "%' or
		tbl_user_master.fridge_price like'%" . $searchValue . "%' or
		tbl_user_master.tv_price like'%" . $searchValue . "%' or
		tbl_user_master.microwave_price like'%" . $searchValue . "%' or
		tbl_user_master.laptop_price like'%" . $searchValue . "%' or
		tbl_user_master.geyser_price like'%" . $searchValue . "%' or
		tbl_user_master.paper_price like'%" . $searchValue . "%' or
		tbl_user_master.iron_price like'%" . $searchValue . "%' or
		tbl_user_master.furniture_price like'%" . $searchValue . "%' or
		tbl_user_master.battery_price like'%" . $searchValue . "%' or
		tbl_user_master.cardboard_price like'%" . $searchValue . "%' or
		tbl_user_master.company_name like'%" . $searchValue . "%' or
	...
	$query = "SELECT 
		...
		WHERE tbl_user_master.user_type = 'Buyer' ";
	...
	while ($row = mysqli_fetch_assoc($empRecords)) {
		...
		## buyer profile data 
		$waste_cate = $row['waste_cate'] == "Skip" ? "NA" : $row['waste_cate'];
		$working_days = $row['working_days'] == "Skip" ? "NA" : $row['working_days'];
		$working_method = $row['working_method'] == "Skip" ? "NA" : $row['working_method'];
		$livelihood_source = $row['livelihood_source'] == "Skip" ? "NA" : $row['livelihood_source'];
		$deal_customer = $row['deal_customer'] == "Skip" ? "NA" : $row['deal_customer'];
		$gst_num = $row['gst_num'] == "Skip" ? "NA" : $row['gst_num'];
		$pan_num = $row['pan_num'] == "Skip" ? "NA" : $row['pan_num'];

		## buyer rate list data
		$ac_price = $row['ac_price'] == "Skip" ? "NA" : $row['ac_price'];
		$washing_machine_price = $row['washing_machine_price'] == "Skip" ? "NA" : $row['washing_machine_price'];
		$fridge_price = $row['fridge_price'] == "Skip" ? "NA" : $row['fridge_price'];
		$tv_price = $row['tv_price'] == "Skip" ? "NA" : $row['tv_price'];
		$microwave_price = $row['microwave_price'] == "Skip" ? "NA" : $row['microwave_price'];
		$laptop_price = $row['laptop_price'] == "Skip" ? "NA" : $row['laptop_price'];
		$geyser_price = $row['geyser_price'] == "Skip" ? "NA" : $row['geyser_price'];
		$paper_price = $row['paper_price'] == "Skip" ? "NA" : $row['paper_price'];
		$iron_price = $row['iron_price'] == "Skip" ? "NA" : $row['iron_price'];
		$furniture_price = $row['furniture_price'] == "Skip" ? "NA" : $row['furniture_price'];
		$battery_price = $row['battery_price'] == "Skip" ? "NA" : $row['battery_price'];
		$cardboard_price = $row['cardboard_price'] == "Skip" ? "NA" : $row['cardboard_price'];
		$company_name = $row['company_name'] == "Skip" ? "NA" : $row['company_name'];
		...
		$data[] = array(
			...
			"authorization_certificate_num" => $row['authorization_certificate_num'],
			"gst_num" => $gst_num,
			"buyer_type" => $row['buyer_type'],
			"address_details" => $address_details,
			"waste_cate" => $waste_cate,
			"working_days" => $working_days,
			"working_method" => $working_method,
			"livelihood_source" => $livelihood_source,
			"deal_customer" => $deal_customer,
			"ac_price" => $ac_price,
			"washing_machine_price" => $washing_machine_price,
			"fridge_price" => $fridge_price,
			"tv_price" => $tv_price,
			"microwave_price" => $microwave_price,
			"laptop_price" => $laptop_price,
			"geyser_price" => $geyser_price,
			"paper_price" => $paper_price,
			"iron_price" => $iron_price,
			"furniture_price" => $furniture_price,
			"battery_price" => $battery_price,
			"cardboard_price" => $cardboard_price,
