...
if ($login == "No") {
	...
	if ($entry_permission == 'Yes') {
		...
		if ($address_id == "") {
			...
			$status = "Save";
			$status_text = "New Buyer Address Data Added Successfully";
		...
		//============================== IF CODE DOES NOT BLANK THEN UPDATE ==================================
						...
						if (!empty($buyer_type)) {
	       mysqli_query($con, "UPDATE tbl_user_master SET buyer_type = '$buyer_type' WHERE user_id = '$user_id'");
}
			...
			$status = "Save";
			$status_text = "Buyer Address Data Updated Successfully";
