$(document).ready(function () {
	...
	make_select2_ajx('buyer_type');
});

function clear_input() {
	_("#address_id").value = "";
	$("#user_id").val('').trigger('change');
	$("#buyer_type").val('').trigger('change');
...
function chngMode(mode) {
	if (mode == "Update") {
		_(".entry_modal_title").innerHTML = 'Update Buyer Address Details :';
		_(".save_btn").innerHTML = 'Update Data';
	}
	else {
		_(".entry_modal_title").innerHTML = 'Enter Buyer Address Details :';
...
function make_select2_ajx(id) {
	...
	$("#" + id).select2({
		...
		width: '100%',
		ajax: {
			url: "templates/manage_buyers_addresses/" + fileName + ".php",
			dataType: 'json',
			type: "post",
			delay: 250,
...
function show_list() {
	if (view_per == "Yes") {
		$('#data_table').DataTable({
			...
			'serverMethod': 'post',
			'ajax': {
				'url': 'templates/manage_buyers_addresses/list.php',
				'data': function (d) {
...
function save_details() {
	...
	save_data = new XMLHttpRequest();
		...
		save_data.open('POST','templates/manage_buyers_addresses/save_details.php',true);	
		save_data.send(data);
	}
}
...
function update_data(address_id) {
	...
	if (edit_per == "Yes") {
		...
		xhr.onreadystatechange = function () {
			if (xhr.readyState == 4) {
				...
				_("#city").value = response['city'];
					_("#buyer_type").innerHTML = `<option value="` + response['buyer_type'] + `" selected>` + response['buyer_type'] + `</option>`;
		...
		xhr.open('POST', 'templates/manage_buyers_addresses/update_data_input.php', true);
		xhr.send(data);
	}
	else {
...
function delete_data(address_id) {
	...
	if (del_per == "Yes") {
		...
		xhr.open('POST', 'templates/manage_buyers_addresses/delete_data.php', true);
		xhr.send(data);
