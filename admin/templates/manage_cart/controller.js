...
function chngMode(mode) {
	if (mode == "Update") {
		_(".entry_modal_title").innerHTML = 'Update Buyer Cart Details :';
		_(".save_btn").innerHTML = 'Update Data';
	}
	else {
		_(".entry_modal_title").innerHTML = 'Enter Cart Details :';
...
function make_select2_ajx(id) {
	...
	$("#" + id).select2({
		...
		width: '100%',
		ajax: {
			url: "templates/manage_cart/" + fileName + ".php",
			dataType: 'json',
			type: "post",
			delay: 250,
...
function show_list() {
	if (view_per == "Yes") {
	  $("#data_table").DataTable({
		...
		serverMethod: "post",
		ajax: {
		  url: "templates/manage_cart/list.php",
		},
		...
		columns: [{ data: "action" }, { data: "buyer_details" }, { data: "product_name" }],
		dom: "lBfrtip",
		buttons: [
...
function save_details(){
	...
	if(!_("#buyer_id").checkValidity()){
		toastr['warning']("Buyer Details : "+_("#buyer_id").validationMessage, "WARNING");
	...
	if(!_("#product_id").checkValidity()){
		toastr['warning']("Product: "+_("#product_id").validationMessage, "WARNING");
	...
	if(save_no==1 && insert_per=="Yes"){
		...
		save_data.open('POST','templates/manage_cart/save_details.php',true);	
		save_data.send(data);
	}
}
...
function update_data(cart_id){
	if(edit_per=="Yes"){
		...
		xhr.onreadystatechange = function() {if(xhr.readyState == 4) {
			...
			const buyer_id = response[0]['buyer_name'] +" ["+ response[0]['ph_num'] +"]";
			const product_id = response[0]['product_name'];
			_("#cart_id").value = response[0]['cart_id'];
		...
		xhr.open('POST','templates/manage_cart/update_data_input.php',true);	
		xhr.send(data);
	}
	else{
...
function delete_data(cart_id) {
	if (del_per == "Yes") {
		...
		xhr.open('POST', 'templates/manage_cart/delete_data.php', true);
		xhr.send(data);
	}
	else {
