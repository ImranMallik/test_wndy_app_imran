...
if ($login == "No") {
    ...
    if ($execute == 1) {
        ...
        if ($rw > 0) {
            $status = "product_name Exist";
            $status_text = "Item Name Already Exists!";
            $execute = 0;
        }
    }
        ...
        if ($product_id == "") {
            ...
            if (mysqli_num_rows($buyersResult) > 0) {
                while ($buyer = mysqli_fetch_assoc($buyersResult)) {
                    ...
                    $sendTextMessage = " Dear '" . $buyer['name'] . "', we have some scrap item(s) posted for sale in your location. Please log on to WNDY and check the app for details - ZAG Tech Solutions";
                    sendWhatsappMessage();
                }
            }
