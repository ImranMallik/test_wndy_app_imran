...
$category_id = isset($sendData['trans_category_id']) ? trim(mysqli_real_escape_string($con, $sendData['trans_category_id'])) : "";

// Base SQL query to fetch purchased items
$query = "
    SELECT 
    ...
    LEFT JOIN 
        ...
        ON pm.product_id = dt.product_id 
    LEFT JOIN 
        tbl_product_file pf 
        ON pf.product_id = dt.product_id AND pf.active = 'Yes' 
    LEFT JOIN 
        tbl_category_master cm 
        ON cm.category_id = pm.category_id AND cm.active = 'Yes' 
    WHERE 
...
<?php } else { ?>
                ...
                <div class="col-4 col-sm-12 col-md-4 mb-1">
                    <div class="product-box position-relative"
                        ...
                        <div class="product-image"
                            style="height: 140px; background: url('upload_content/upload_img/product_img/<?php echo htmlspecialchars($row['file_name'] ?: 'no_image.png'); ?>') no-repeat center center/cover;">
                        </div>

                        <!-- Product Details -->
                        <div class="product-details text-center"
                            style="background-color: #2f415d; color: #fff; padding: 4px; position: absolute; bottom: 0; width: 100%;">
                            <div class="product-name" style="font-size: 14px; font-weight: 500; margin: 0; line-height: 1.5;">
                                <?php
                                echo !empty($row['product_name'])
                                    ? (mb_strlen($row['product_name']) > 20
                                        ? htmlspecialchars(mb_substr($row['product_name'], 0, 20)) . '...'
    ...
    <div style="position: fixed; bottom: 0; left: 0; width: 100%;">
        <button id="acceptButton" onclick="acceptProduct('<?php echo $session_user_code; ?>')"
            style="
            width: 100%; 
            background-color: #28a745; 
            color: white; 
            border: none; 
            border-radius: 0; /* Optional: For seamless full-width button */
            padding: 15px 0; 
            cursor: pointer;
        ">
            Accept All
