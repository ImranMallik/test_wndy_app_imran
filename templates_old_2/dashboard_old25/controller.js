...
function makeSellerScrapValuationChart() {
    // Ensure bars are visible, even for small values
    const adjustedValues = sellerScrapValuationValue.map(val => val <= 1 ? 5 : val);
    ...
    var options = {
        ...
        plotOptions: {
            bar: {
                columnWidth: '50%', // Adjust for visibility
                barHeight: '100%',
                distributed: true,
            }
        },
        ...
        yaxis: {
            min: 0,
            max: Math.max(...sellerScrapValuationValue) * 1.2, // Dynamically adjust scale
        }
    };
