...
if ($login == "No") {
    ...
    $rating_buyer_id = mysqli_real_escape_string($con, $sendData['rating_buyer_id']);
    ...
    if ($execute == 1) {
        $dataget = mysqli_query($con, "SELECT * FROM tbl_ratings WHERE give_user_id='" . $session_user_code . "' AND to_user_id='" . $rating_buyer_id . "' AND rating_from='Seller' AND view_id='" . $rating_view_id . "'");
        $data = mysqli_fetch_row($dataget);
        if ($data) {
            $status = "error";
            $status_text = "You Have Already Rated The Buyer";
            $execute = 0;
        }
    }
