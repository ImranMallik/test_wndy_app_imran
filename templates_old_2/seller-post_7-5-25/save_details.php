...
LEFT JOIN tbl_product_file f ON p.product_id = f.product_id
WHERE p.product_id = '$product_id'
LIMIT 1");

            $row = mysqli_fetch_assoc($productNameDataGet);
            $productName = $row['product_name'];
            $product_image = $row['file_name'];
...
WHERE tbl_address_master.pincode = '$pincode' AND tbl_user_master.user_type = 'Buyer'";
    ...
    if (mysqli_num_rows($buyersResult) > 0) {
        while ($buyer = mysqli_fetch_assoc($buyersResult)) {
            ...
            $img_url = $baseUrl . "/upload_content/upload_img/product_img/" . urlencode($product_image2);
            $params =
                [
                    $fetchUserName,
                    $Link,
